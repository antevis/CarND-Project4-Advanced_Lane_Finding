...
class LineDetector:
    def __init__(self, imgMarginWidth=320, historyDepth=5, margin=100, windowSplit=2, winCount=9, searchPortion=1.):
    ...
    def addPip(self, pipImage, dstImage, pipAlpha=0.5, pipResizeRatio=0.3, origin=(20, 20)):
        ...
        backGround = dstImage[origin[0]:origin[0] + pipHeight, origin[1]:origin[1] + pipWidth]

        blend = np.round(backGround * (1 - pipAlpha), 0) + np.round(smallPip * pipAlpha, 0)

        blend = np.minimum(blend, 255)

        dstImage[origin[0]:origin[0] + pipHeight, origin[1]:origin[1] + pipWidth] = blend

        return dstImage

    def addOffsetStamp(self, leftFit, rightFit, image, origin, color=(255, 255, 255), fontScale=1.0):
        """
    ...
    def embedLane(self, src, pipParams=None):
        """
        Main 'pipeline' for adding Lane polygon to original image
        ...
        if pipParams is not None:
            ...
            pipWidth = int(commonBin.shape[1] * ratio)

            img = self.addPip(pipImage=commonBin, dstImage=img,
                              pipAlpha=alpha, pipResizeRatio=ratio, origin=origin)

            # Updating origin for telemetry if PiP has been added
            stampOriginX = pipWidth + 40

        self.addCurvatureStamp(leftFit=currFitLeft, rightFit=currFitRight,
                               image=img, origin=(stampOriginX, 40), fontScale=0.66)

        self.addOffsetStamp(leftFit=currFitLeft, rightFit=currFitRight,
                            image=img, origin=(stampOriginX, 80), fontScale=0.66)
...
def main():
    ...
    margin = aux.promptForInt(message='Enter Search Margin: ')
    ...
    ld = LineDetector(imgMarginWidth=fillerWidth, historyDepth=depth,
                      margin=margin, windowSplit=windowSplit, winCount=winCount, searchPortion=searchPortion)

    for frame in tqdm(clip.iter_frames()):
        dst = ld.embedLane(src=frame, pipParams=pipParams)
    ...
    resultFileName = '{}_out.mp4'.format(resultFileName)
